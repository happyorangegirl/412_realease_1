...
from ehc_config_adapter.json_schema_generator.pattern_generator import PatternGenerator
...
class SchemaGenerator(object):
    ...
    def to_dict(self, base_object=None, object_id=None, first_level=True, required = True, nullable = False):
        ...
        if first_level:
            ...
            schema_dict["title"] = "EHC E2E Automation Configurations"
            schema_dict["id"] = "#"

        if object_id is not None:
            schema_dict["id"] = str(object_id)
            # schema_dict["description"] = str(object_id).replace('_', ' ')
        ...
        if schema_type == ObjectType and len(base_object) > 0:
            ...
            if same_type:
                schema_dict["type"] = Type.get_schema_type_for(first_item_type).json_type
                schema_dict['enum'] = base_object
            ...
            schema_dict['pattern'] = PatternGenerator.get(object_id,schema_type)
            #full_msg = {pattern:msg}
            #schema_dict['message']=msg
            schema_dict['default'] = base_object
