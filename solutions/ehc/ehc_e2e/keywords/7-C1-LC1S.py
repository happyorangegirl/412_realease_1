...
WF_SETTINGS = 'E2EWF-7-C1-LC1S.config.yaml'
...
def main():
    ...
    for i in aucs:
        ...
        if i.__name__ in ('cloud_administrator_displays_backup_service_level',
                          'cloud_administrator_edits_avamar_grid_admin_full',
                          'cloud_administrator_run_admin_report',
                          'cloud_administrator_edits_avamar_grid'):
            try:
                func()
            ...
            if i.__name__ == '':
                log.debug('added_avamar_site_relationship: %s' % wf.wf_context.added_avamar_site_relationship)
            func()
